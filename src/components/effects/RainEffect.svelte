<script>
  ...
  export let solidColor = SolidColors.CYAN;
    ...
    try {
      ...
      for (let frame = 0; frame < 25; frame++) {
        ...
        for (let i = 0; i < characters.length; i++) {
          ...
          if (frame > char.revealFrame) {
            const rainProgress = (frame - char.revealFrame) / 18;
            if (rainProgress < 1) {
              char.symbol = randomSymbol();
              char.color = `rgba(0, 200, 255, ${Math.max(0, 1 - rainProgress * 0.6)})`;
            } else {
              char.symbol = char.char;
              char.color = solidColor;
              char.fontWeight = calculateFontWeight(progress, i, characters.length);
              char.textShadow = calculateShiningShadow(progress, i, characters.length, solidColor);
            }
          }
        }
        characters = [...characters];
