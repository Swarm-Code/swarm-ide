<script>
  ...
  export let solidColor = SolidColors.BLUE;
  ...
  async function animate() {
    ...
    try {
      ...
      for (let frame = 0; frame < 20; frame++) {
        const wavePhase = (frame / 20) * Math.PI * 2;
        for (let i = 0; i < characters.length; i++) {
          const char = characters[i];
          const charWave = ((char.x / 15) - wavePhase) * Math.PI * 2;
          char.yOffset = Math.sin(charWave) * 4;
      ...
      for (let frame = 0; frame < 20; frame++) {
        ...
        for (let i = 0; i < characters.length; i++) {
          const char = characters[i];
          const charWave = ((char.x / 15) - wavePhase) * Math.PI * 2;
          const waveAmount = Math.sin(charWave);
          char.yOffset = waveAmount * 3;

          if (Math.abs(waveAmount) < 0.3) {
  ...
  {#each asciiLines as line, y}
    <div class="line">
      {#each line.split('') as _, x}
        ...
        {#if char}
          <span style="color: {char.color}; font-weight: {char.fontWeight}; text-shadow: {char.textShadow}; transform: translateY({char.yOffset}px);">{char.symbol}</span>
        {:else}
          <span>{_}</span>
        {/if}
      {/each}
