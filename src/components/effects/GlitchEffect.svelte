<script>
  ...
  export let solidColor = SolidColors.MAGENTA;
  ...
  async function animate() {
    ...
    try {
      ...
      for (let frame = 0; frame < 20; frame++) {
        const glitchIntensity = Math.sin((frame / 20) * Math.PI) * 0.7;
        for (let i = 0; i < characters.length; i++) {
          const char = characters[i];
          if (Math.random() < glitchIntensity) {
            char.symbol = randomSymbol();
            char.xOffset = (Math.random() - 0.5) * 15 * glitchIntensity;
            char.yOffset = (Math.random() - 0.5) * 12 * glitchIntensity;
          }
        }
        characters = [...characters];
      ...
      for (let frame = 0; frame < 20; frame++) {
        ...
        const stability = progress * progress;

        for (let i = 0; i < characters.length; i++) {
          const char = characters[i];
          if (Math.random() < (1 - stability) * 0.2) {
            char.symbol = randomSymbol();
            char.xOffset = (Math.random() - 0.5) * 5 * (1 - stability);
            char.yOffset = (Math.random() - 0.5) * 4 * (1 - stability);
          } else {
            char.symbol = char.char;
            char.color = solidColor;
            char.xOffset = char.xOffset * 0.8;
            char.yOffset = char.yOffset * 0.8;
      ...
      for (let frame = 0; frame < shineFrames; frame++) {
        ...
        for (let i = 0; i < characters.length; i++) {
          ...
          characters[i].xOffset = 0;
          characters[i].yOffset = 0;
        }
        characters = [...characters];
  ...
  {#each asciiLines as line, y}
    <div class="line">
      {#each line.split('') as _, x}
        ...
        {#if char}
          <span style="color: {char.color}; font-weight: {char.fontWeight}; text-shadow: {char.textShadow}; transform: translate({char.xOffset}px, {char.yOffset}px);">{char.symbol}</span>
        {:else}
          <span>{_}</span>
        {/if}
      {/each}
